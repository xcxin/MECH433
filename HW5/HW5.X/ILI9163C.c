...
void draw_string(const char *str, int x, int y) {
    ...
    while(str[i] != '\0') {
        draw_char(str[i], x, y);
        x = x + 6;  //characters are 5 pixels wide, should fit into one row
        i++;
    }
...
void draw_char(char c, int x, int y) {
  int i;
  for (i=0; i<5; i++) {
    char byte = ASCII[(int)(c) - 0x20][i];//c in its decimal minus the first 0x20
    int j;
    int bit;
    for (j=7; j>=0; j--) {
        bit=byte>>j&1;
        if (x+i>=0 && x+i<129 && y+j>=0 && y+j<129){
            if (bit==1){
